...
Fila *fila_cria(int tam_do_dado) {
  ...
  if (self!= NULL) {
    *self = malloc(sizeof(struct _fila));
    if (*self!= NULL) {
      (*self)->max_elem = malloc(sizeof(int));
      *(*self)->max_elem = MAX_ELEM_INICIAL;
      (*self)->espaco = malloc(*(*self)->max_elem * tam_do_dado);
      if ((*self)->espaco!= NULL) {
        (*self)->n_elem = 0;
...
void fila_insere(Fila *self, void *pdado) {
  assert((*self)->n_elem < *(*self)->max_elem);
  (*self)->n_elem++;
